...
class TestCancerLesionSharedBrowser(SQLiteSerializedStaticLiveServerTestCase):
    ...
    def test_combined_selected_content_and_lesion_change_clear_phone_body(self):
        ...
        with self.browser(reader, 360) as page:
            status_url = self.live_server_url + f'/shared/{identity}/status/'
            # This live page starts a status request on pageshow; network idle
            # is not the readiness contract for an active access monitor.
            with page.expect_response(lambda response: response.url == status_url) as initial_status:
                response = page.goto(self.live_server_url + f'/shared/{identity}/', wait_until='domcontentloaded')
            self.assertEqual(response.status, 200)
            self.assertEqual(initial_status.value.status, 200)
            ...
            with page.expect_response(lambda response: response.url == status_url) as changed_status:
                page.evaluate("window.dispatchEvent(new Event('pageshow'))")
            self.assertEqual(changed_status.value.status, 410)

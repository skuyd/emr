...
def lock_sources(patient, document_ids):
    ...
    locked_patient = Patient.objects.select_for_update().filter(pk=patient.pk, account__is_active=True).first()
...
def build_snapshot(patient, selection, *, now=None):
    ...
    with transaction.atomic():
        if Patient.objects.select_for_update().filter(pk=patient.pk, account__is_active=True).first() is None:
            raise PermissionDenied

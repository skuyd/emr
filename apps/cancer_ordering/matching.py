...
MATCHING_VERSION = "reported-cancer-literals-2"
...
LEFT_BOUNDARY = re.compile(
    r"(?:[、,/]|但是|然而|不过|但|而|以及|合并|伴|及|和|与|或者|或|患者|"
...
RIGHT_BOUNDARY = re.compile(
    ...
    r"未(?:见|检出|检测到|发现)|否认|术后|治疗后|合并|伴|及|和|与|或|转移|均|皆|两者|二者)"
)
...
OTHER_PERSON = re.compile(r"父亲|母亲|父母|兄弟|姐妹|祖父|祖母|外祖|家族史|他人|丈夫|妻子|配偶|儿子|女儿|兄长|哥哥|弟弟|姐姐|妹妹")
...
GROUP_END = re.compile(CONTRAST.pattern + r"|[,，]")
CURRENT_PREFIX = re.compile(
    r"(?:" + LITERAL.pattern + "|" + COORDINATION.pattern
    + r"|患者|从未患|未患|患有|患|确诊为|诊断为|明确|提示|考虑|倾向|可能|可疑|疑似|"
      r"不能排除|无法排除|不能除外|不除外|待排|排除|未见|未发现|未检出|未证实|否认|无|[,，()])*"
)
...
def _subject(text):
    ...
    literals = list(LITERAL.finditer(text))
    prefix = ORDINAL.sub("", text[:literals[-1].start()]) if literals else None
    # Only a complete supported prefix supplies an implicit patient subject.
    # An inner 患/诊断为 after an unknown person or predicate cannot do so.
    if prefix is None or not CURRENT_PREFIX.fullmatch(prefix):
        return Subject.UNKNOWN.value
...
def _context(body, start, end):
    ...
    separators = list(GROUP_END.finditer(body[:start]))
    group_start = separators[-1].end() if separators else 0
    group_boundary = GROUP_END.search(body, end)
    group_end = group_boundary.start() if group_boundary else len(body)
    group = body[group_start:group_end]
    shared = re.search(r"(?:均|皆|两者|二者).*$", group)
    shared_assertion = None
    if shared and len(MALIGNANCY_WORD.findall(group[:shared.start()])) >= 2 and COORDINATION.search(group[:shared.start()]):
        qualified = _assertion(shared.group())
        # A shared affirmative phrase does not cancel an enclosing preliminary
        # heading, disjunction, or already scoped negative predicate.
        shared_assertion = qualified if qualified != Assertion.AFFIRMED.value else None
    return body[begin:finish], ambiguous_scope, shared_assertion


def _source_row(raw, positions, left, right, match_left, match_right, *, label, profile, context,
                preliminary=False, disjunctive=False, ambiguous_scope=False, shared_assertion=None):
    start, end = positions[left], positions[right - 1] + 1
    match_start, match_end = positions[match_left], positions[match_right - 1] + 1
    return {"label": label, "profile": profile, "assertion": shared_assertion or _assertion(context, preliminary=preliminary,

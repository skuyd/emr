...
def combined_extractor_version():
    """Identify every component actually run by the shared extraction pipeline."""
    from .pathology_extraction import EXTRACTOR_VERSION as pathology_version
    from .molecular_extraction import EXTRACTOR_VERSION as molecular_version

    return "+".join((EXTRACTOR_VERSION, pathology_version, molecular_version))
...
def extract_clinical_version(version, *, construction_context=None):
    ...
    with transaction.atomic():
        ...
        from .molecular_extraction import molecular_candidates, persist_molecular_candidates
        from .molecular_segments import segment_molecular_reports, SEGMENTER_VERSION as MOLECULAR_SEGMENTER
        from .molecular_schema import SCHEMA as MOLECULAR_SCHEMA

        segments, unparsed = segment_reports(blocks)
        pathology_segments, _ = segment_pathology_reports(blocks)
        molecular_segments, _ = segment_molecular_reports(blocks)
        segments.extend(pathology_segments)
        segments.extend(molecular_segments)
        segments.sort(key=lambda segment: (segment.pieces[0].page, segment.pieces[0].block.reading_order, segment.pieces[0].start))
        unparsed -= {piece.page for segment in [*pathology_segments, *molecular_segments] for piece in segment.pieces}
        ...
        for ordinal, segment in enumerate(segments):
            ...
            molecular = getattr(segment, "routing_kind", "IMAGING") == "MOLECULAR"
            molecular_group = molecular_candidates(segment) if molecular else None
            ...
            report = ClinicalReport(
                ...
                routing_kind="MOLECULAR" if molecular else "PATHOLOGY" if pathology else "IMAGING",
                segmenter_version=MOLECULAR_SEGMENTER if molecular else PATHOLOGY_SEGMENTER if pathology else SEGMENTER_VERSION,
                schema_version=MOLECULAR_SCHEMA if molecular else PATHOLOGY_SCHEMA if pathology else SCHEMA_VERSION, source_fingerprint=fingerprint,
            ...
            for span_order, piece in enumerate(segment.pieces):
                ...
                span.save()
            if molecular:
                count += persist_molecular_candidates(report, molecular_group, construction_context=construction_context)
            else:
        ...
        return ClinicalExtraction.objects.create(
            parsing_version=version, extractor_version=combined_extractor_version(), schema_version=SCHEMA_VERSION,

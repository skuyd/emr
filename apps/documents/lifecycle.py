...
def _owned_document(patient, document_id, actor=None, capability="write"):
    try:
        authorize_patient(patient, owner_actor(patient, actor), capability, lock=True)
    except PermissionDenied:
        if not Patient.objects.filter(pk=patient.pk, deleted_at__isnull=True, account__is_active=True).exists():
            raise LifecycleUnavailable("资料不可用。") from None
        raise
...
def move_to_trash(patient, document_id, *, actor=None, now=None):
    with transaction.atomic():
        document, batches = _owned_document(patient, document_id, actor)
        now = _now(now)
        ...
        revoke_document_reviews(document, actor=authorize_patient(patient, owner_actor(patient, actor), "write").actor, action="DOCUMENT_TRASHED")
        ...
        record_audit_event(getattr(owner_actor(patient, actor), "pk", owner_actor(patient, actor)), "document_trashed", document.pk, "succeeded", "recoverable")
    return document


def restore_document(patient, document_id, *, actor=None, now=None):
    with transaction.atomic():
        document, _batches = _owned_document(patient, document_id, actor)
        now = _now(now)
        ...
        document.lifecycle_revision += 1
        document.save(update_fields=["deleted_at", "trashed_at", "trash_expires_at", "lifecycle_revision", "updated_at"])
        record_audit_event(getattr(owner_actor(patient, actor), "pk", owner_actor(patient, actor)), "document_restored", document.pk, "succeeded")
    return document


def permanently_delete_from_trash(patient, document_id, *, dispatch, actor=None, now=None):
    with transaction.atomic():
        document, _batches = _owned_document(patient, document_id, actor)
        now = _now(now)
        if document.trashed_at is None:
            raise LifecycleUnavailable("回收站资料不可用。")
        return request_document_deletion(patient, document.pk, actor=actor, dispatch=dispatch, now=now)

...
| --- | --- | --- | --- | --- | --- |
| Chrome 152.0.7977.75（Windows） | 已自动验证 | 已自动验证 | 已验证 | 已验证 | 本机完成 |
| Edge 152.0.4191.62（Windows） | 已自动验证 | 已自动验证 | 已验证 | 已验证 | 本机完成 |
...
2026-09-05 本机预演结果：Chrome `6 passed in 43.28s`，Edge
`6 passed in 37.41s`；两次运行均覆盖登录、建档、上传、返回首页、密码重置、旧账号升级、
通知与响应式外壳，并检查失败响应、控制台错误和横向溢出。完整命令与版本见
[本机门禁预演制品](artifacts/local-gate-rehearsal-result.json)。

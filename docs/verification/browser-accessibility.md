...
新增的 `tests/e2e/phr-v1.spec.ts` 已由 Playwright 成功发现并编译，共 27 个项目化用例，覆盖：

- P00 登录/隐私、P01 三项同意、P02 首页、P03 上传；
- P04 搜索、P05 详情、P06 原件首屏、P07 趋势、P08 设置；
- 合成文档二次确认删除及删除后 404；
- 1280×720 与 1440×900 无横向溢出；
- 页面地标、标签、可见键盘焦点、文字状态和首页/病案可交互时间。
...
正式套件缺少上述任何 HTTPS 地址、隔离会话或合成样本输入时会直接失败，不会以 skipped
状态返回成功。`npx playwright test --list` 只证明用例可编译和被发现，不属于浏览器验收证据。

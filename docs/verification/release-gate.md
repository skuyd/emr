...
生成时间：`2026-08-31T05:04:57+08:00`。必需门禁 23 项，通过 7 项，待验证 16 项。
...
| Chrome 当前主版本完整流程 | pending | 当前版本冒烟已通过，但正式 HTTPS 环境 P00–P08 完整 Playwright 套件未运行 | `docs/verification/browser-accessibility.md` |
| Chrome 前一主版本完整流程 | pending | 缺少该历史主版本测试环境 | — |
| Chrome 前二主版本完整流程 | pending | 缺少该历史主版本测试环境 | — |
| Edge 当前主版本完整流程 | pending | 当前版本冒烟已通过，但正式 HTTPS 环境 P00–P08 完整 Playwright 套件未运行 | `docs/verification/browser-accessibility.md` |
| Edge 前一主版本完整流程 | pending | 缺少该历史主版本测试环境 | — |
| Edge 前二主版本完整流程 | pending | 缺少该历史主版本测试环境 | — |
| Safari 当前主版本完整流程 | pending | 本机没有 macOS/Safari 真机；WebKit 参考不能替代 | `docs/verification/external-compatibility.md` |
| Safari 前一主版本完整流程 | pending | 缺少对应的受安全支持 macOS/Safari 真机 | `docs/verification/external-compatibility.md` |
| 跨浏览器无障碍人工验收 | pending | 需在全部受支持真实浏览器完成键盘、读屏、200% 缩放和高对比度检查 | `docs/verification/browser-accessibility.md` |
| PostgreSQL 并发事务验证 | pending | 本机没有可用 PostgreSQL 服务，2 个真实并发用例仍跳过 | `tests/integration/test_processing_postgres_concurrency.py` |
| 离线 PaddleOCR 模型实跑 | pending | 缺少已校验的本地 Paddle 模型目录，模型烟测仍跳过 | `tests/processing/test_paddle_adapter.py` |
| 真实生产短信网关 | pending | 尚未提供正式短信网关域名、API 凭据和模板 | `tests/accounts/test_sms_gateway.py` |
| 真实私有 S3 | pending | 尚未连接真实版本化/加密/公共访问阻断 S3 桶执行启动探针 | `tests/deploy/test_private_storage_check.py` |
| 多进程 Web/Worker/Beat 部署 | pending | 本机未安装 Docker，尚未构建并运行完整生产编排 | `deploy/compose.yaml`<br>`deploy/Dockerfile` |
| 固定负载性能 | pending | 本机无 k6 和固定容量环境，P50/P95/P99 及 5 分钟结果尚无真实数据 | `tests/performance/k6-upload-search.js`<br>`docs/verification/performance.md` |
| 加密备份恢复与删除账本演练 | pending | 本机无 Docker/age/真实 S3，RPO≤15 分钟和 RTO≤4 小时尚未实测 | `deploy/backup.ps1`<br>`deploy/restore.ps1`<br>`docs/verification/backup-restore.md` |
...
- 离线 PaddleOCR 模型实跑：缺少已校验的本地 Paddle 模型目录，模型烟测仍跳过

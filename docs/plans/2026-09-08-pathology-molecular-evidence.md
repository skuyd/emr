...
设计正在独立评审，登记为 `draft / implementing`；本文的后续任务尚未执行。
...
后的最新 main 创建新分支。当前只冻结设计和 27 页原件覆盖，独审通过前不实施下面的解析任务。
...
2. 将可用新字段接入搜索、速查和既有 CSV/JSON/ZIP，使用实际 HTTP 生成并打开产物检查。
   细选 IHC 评分不能夹带未选标记、整报告、旧摘录或完整原件。

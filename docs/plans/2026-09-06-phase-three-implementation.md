...
- 以上为开发过程中的局部执行记录。随后已完成 Task 6，当前结论见
  [第三阶段验证记录](../verification/phase-three.md)及[汇总制品](../verification/artifacts/phase-three-verification-report.json)。
- 原件标注固定覆盖 64 文件、60 报告组、124 页的清单，232 个可判断字段；34 页未作完整
  事实标注、2 个截断字段无法判断，均保留范围说明。编码代理直接查看原件，未进行独立
  临床专家复核。真实候选 60 正确、15 错配、157 漏提取、1 额外，精确率 60/76、召回率
  60/232；12 个合成字段全部符合固定契约，结果分别报告。低质量 OCR 和药品医嘱仍需
  人工补录，不声称总体医学准确率或独立泛化能力。
- 实际执行 3 份原件、6 条原件标注事实的人工核对服务到 PDF/JSON/CSV/ZIP 链路，原文、
  不同来源分期及 3 份原件哈希保留；两页 PDF 已逐页查看。私有制品不进入仓库。
- 256 MiB 合成原件的生成、下载和取消清理通过；导出改为分块校验及私有磁盘临时文件。
  生产配置补充私有磁盘卷，避免 1 GB 内存临时目录限制大包导出。
- 完整 Python 回归 1526 passed、2 skipped、40 deselected（292.11 秒）；两个跳过均为
  Windows 缺少符号链接权限。随后导出临时目录变更执行导出、部署和核心检查 168 passed
  （19.84 秒），两次范围重叠，不相加。必跑浏览器 8 passed、PostgreSQL 本期并发 4 passed，
  早前 PostgreSQL 全套 39 passed。JavaScript、Django、迁移、版本、文档与追踪校验见汇总。
- AC01–AC16 本地功能验收已逐项记录，实现提交 `a887270`、`7f24789`、`c6d651f`、

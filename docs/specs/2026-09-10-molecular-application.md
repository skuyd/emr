...
本分支初始作者采用该基线下一候选 portable `1.5`；实际主线 `c6a3ae30` 已发布的云影像
使用 `1.5` 和两张云来源数组。本分支现正常合流该实际主线，分子内容沿临床三数组
扩展并使用下一 portable `1.6`，保留云来源和其他已发布数组及 `1.0–1.5` reader。
交付前仍按届时实际 main 再核对合法值，不导入未合分支，也不把应用发布号当格式版本。

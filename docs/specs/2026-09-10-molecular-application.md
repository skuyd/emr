...
应用层对检测种类和上述独立断言执行有限原词到代码的一致性校验；例如实印“小变异”
不能被保存为 COPY_NUMBER，实印“阳性”不能被保存为 NEGATIVE。检测范围的原词、
目标和限制均须出现在本字段自己的值片段，更正入口也检查；自动断言不能借复制锚
或标签片段作自身依据。无法映射的词保留未分类原文，不生成报告 UNCERTAIN 或
NOT_PROVIDED；数值仍为无推断状态。UNKNOWN 来源判断与原报告“不确定”严格区分。
此校验只匹配有限明示词，不查询医学知识、不使用阈值，也不改变 A0 的 28 字段合同。
独立断言必须匹配完整陈述的有限语法，不能以包含关键词或否定词黑名单建立结果。
`none detected`、未知前后修饰及复合断言保持未分类，不能转为原文明示检出或不确定。
实际来源同样校验：不能从 `not negative` 只截取 `negative`；换行和冒号不自动成为
新断言边界。自动字段依真实 OCR 偏移回到同报告的原陈述，仅以空白分开的原文行跨度
须合并核对；人工转录按完整陈述核对。多个匹配窗口不能唯一支持同一结果时拒绝该断言。
窗口过宽、无明确陈述边界或不能解释修饰时，自动提取保留报告原文与未分类限制，
不把自填缩短原词当成已证明的来源，不为提取率补造结论。
MSI 类别、TMB 定性及药物依据方向同样核对完整原词、实际陈述窗口和有限代码，补录、
普通更正及已有修订的当前可用性均检查。不能将 MSI-L 改成 MSI_H、低改成 HIGH、
耐药改成获益，也不能从 `not` 修饰的原句只取后半词。MSI/TMB 的 UNKNOWN 表示应用
尚未分类，仍须保留完整原词；药物方向无法归入有限原词时保留在原文陈述中，不转成
报告“未说明”或“不确定”。自动来源窗口不支持分类时只保留完整报告及明确限制，不使
整份报告的其他可核验字段丢失。此应用来源约束不修改 A0 值形状或标准化原始字符串。
“报告耐药”与“原报告耐药”都是已支持的明确报告原词；前后否定、不能排除或不支持的
修饰必须随完整窗口核对，不能只取其中的“报告耐药”来授予耐药代码。
自动表格须从同报告真实跨度重建明确表头、对应列和完整单元格后核对分类；不能把任意
分隔符或声明的短片段当成单元格边界。完整单元格中的否定和未分类原词保持原样。
...
自动多片段的首屏定位使用本字段实际原值片段，其他组件逐段可打开；没有整组多边形时
不捏造联合框。浏览器 textarea 的换行传输若与原值仅 CRLF/LF 不同，确认仍保留最初
原字符串；原字词有修改时仍须更正操作。融合的独立基因、转录本及位置组件必须属于
完整身份中实际有序伙伴的对应原组件，不把伙伴改成无序列表或补造方向。
原值控件不得去除首尾空白；多值控件原样提交时保留每个原列表项及项内换行，真实编辑
仍按明示项分隔解析，不能悄悄恢复成旧值。核对页面在最后来源复读之后再次核验当前
权限和原件存活；无效 POST 的错误表单也不能在此窗口撤权或删除后返回旧原文。
授权资料搜索包含完整身份的转录本、密码子、位置及融合两侧组件，沿用当前报告与
未排除字段的来源过滤，不将搜索命中视为当前可用结果。核对路由在 session、CSRF 和
授权之前设置错误报告脱敏，省略原文、请求参数、异常消息与局部变量值，同时保留
异常类别、原因链结构、调用栈、路由名和请求审计编号；同源 CSRF 行为保持。
...
本分支初始候选1.5以及8a54的候选1.6仅为未发布检查点。最终32c6正常合入实际
癌症main9cc（portable1.7），新分子单元采用portable **1.8**；全部已发布云/病灶/癌症数组
保留。reader接受真实旧1.6的非空云/病灶与1.7非空癌症；旧版本1.0–1.7携非空分子单元拒绝，
不能把未发布草稿1.6当兼容义务。应用发布号仍交Release Please实际确定。

共享提取完成身份由实际临床/病理/分子三个组件版本组成；类型化癌症来源兼容已发布的
旧两组件完整身份，未知、缺项、额外后缀、FAILED/PARTIAL不能变成已完成收集。
ClinicalExtraction身份字段通过0007显式扩为128，旧记录和确认不重写；真实PG验证旧短值
往返保真，以及长值存在时拒绝有损反向收窄。此处不是旧A0或FactExtraction值合同变更。
...
完整应用从病理 PR 70 实际 Squash `4b73d2e` 建立独立分支，已实现 M1–M6 的字段/数据库、
自动候选与实际 worker、来源上下文、核对修订/搜索、选定输出/分享和严格 reader。
固定源码 `8a54e18f43c9f933a7b1c6e022e64c998211592d` 的组合普通回归 1459 通过，
PG 正常 COMMIT 边界 72、SQLite 原生 TLS 浏览器 4、PG 原生 TLS 浏览器 4 均零跳过；
各矩阵重叠，不相加为新验收数。A0 28 键与共享日期旧形状仍不变，A0 本身不是完整应用。
